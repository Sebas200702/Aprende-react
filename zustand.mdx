...
**Zustand** es una librería minimalista para manejar el estado global de tu aplicación React. Es mucho más simple que Redux y perfecta para comenzar.

## ¿Qué es Zustand?

<Info>
  **Piensa en ello así**: Zustand es como una **caja fuerte compartida** donde todos los componentes de tu app pueden guardar y consultar información sin necesidad de pasar props por todos lados.
</Info>
...
function Nivel1({ usuario, setUsuario }) {
  return <Nivel2 usuario={usuario} setUsuario={setUsuario} />;
}

function Nivel2({ usuario, setUsuario }) {
  return <Nivel3 usuario={usuario} setUsuario={setUsuario} />;
}

function Nivel3({ usuario, setUsuario }) {
  // Finalmente usamos el usuario aquí
...
```typescript
// ✅ Cualquier componente accede directamente al estado
function Nivel3() {
...
<Tip>
  No necesitas providers ni contextos. Solo importa el hook y úsalo. ¡Así de simple!
</Tip>
...
```typescript
// ✅ Solo se re-renderiza si 'cuenta' cambia
const cuenta = useContadorStore(state => state.cuenta);

// ❌ Se re-renderiza con cualquier cambio en la store
...
export const useCarritoStore = create<CarritoStore>((set, get) => ({
  ...
  // Computed value: se calcula automáticamente
  get total() {
...
<Info>
  Con `persist`, el estado se guarda automáticamente en localStorage y se recupera al recargar la página. ¡Magia!
</Info>
...
Puedes tener varias stores para diferentes propósitos:
...
<Tip>
  **Divide tu estado**: Una store por dominio (auth, carrito, notificaciones, etc.). Es más fácil de mantener.
</Tip>
...
export const createCarritoSlice = (set, get) => ({
  items: [],
  agregarProducto: (producto) => set((state) => ({
    items: [...state.items, { producto, cantidad: 1 }]
  })),
...
export const createUsuarioSlice = (set, get) => ({
  usuario: null,
  login: async (credentials) => { /* ... */ },
  // más acciones...
});
...
| Tamaño | ~1KB 🪶 | ~9KB 📦 | Built-in |
| Curva de aprendizaje | Fácil 😊 | Difícil 😰 | Media 🤔 |
| Boilerplate | Casi nada ✨ | Mucho 📄📄📄 | Medio 📄 |
| Performance | Excelente 🚀 | Buena ✅ | Puede mejorar ⚠️ |
| DevTools | ✅ | ✅ | ❌ |
...
Has completado la guía de React. Ahora conoces:

✅ Qué es React y cómo funciona  
✅ Cómo crear componentes reutilizables  
✅ Interfaces para tipar tus datos  
✅ Props para pasar información  
✅ Organización con imports/exports  
✅ Hooks para agregar funcionalidad  
✅ Custom hooks para reutilizar lógica  
✅ Zustand para estado global  

<Card title="Volver al inicio" icon="house" href="/introduccion">
  Revisa cualquier tema que necesites repasar →
</Card>

<Tip>
  **Próximos pasos**: Practica construyendo proyectos reales. La mejor forma de aprender es haciendo. ¡Éxito! 🚀
</Tip>
